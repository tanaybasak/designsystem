...
class ContentSwitcher {
    ...
    toggleSwitch = (target) => {
        let panel = '';
        Array.from(this.element.querySelectorAll(this.selectors.contentswitcherBtnAll)).forEach((item) => {
            if (item) {
    ...
    selectSwitch = (target) => {
        const switchItem = Array.from(this.element.querySelectorAll(this.selectors.contentswitcherBtnAll)).find((item, index) => {
            return target === index;
        });
        let dataTarget = '';
        if (switchItem && this.element.contains(switchItem)) {
            ...
            if (this.state.disabled && this.state.disabled.length > 0) {
                Array.from(this.element.querySelectorAll(this.selectors.contentswitcherBtnAll)).forEach((item, index) => {
    ...
    toggleSwitchPanel = (switchTarget) => {
        let panel = document.querySelector(switchTarget);
    ...
    clickEventListener = (ev) => {
        ...
        const selectedIndex = Array.from(this.element.querySelectorAll(this.selectors.contentswitcherBtnAll)).findIndex((item) => {
    ...
    attachEvents = () => {
        const contentswitcher = Array.from(this.element.querySelectorAll(this.selectors.contentswitcherBtnAll));
        const len = contentswitcher.length;
        for (let i = 0; i < len; i++) {
            contentswitcher[i].addEventListener('click', this.clickEventListener.bind(this));
        }
    }
    ...
    static handleDataAPI = () => {
        handleDataBinding("contentswitcher", function (element, target) {
            ...
            if (element && target) {
                ...
                idx = Array.from(element.querySelectorAll(`.${PREFIX}-content-switcher-btn`)).findIndex((item) => {
